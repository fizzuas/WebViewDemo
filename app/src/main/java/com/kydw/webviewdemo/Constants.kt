package com.kydw.webviewdemo


//intent
const val KEYWORD_SITES = "KEYWORD_SITES"

//0-无限次
const val CIRCLE_COUNT = "CIRCLE_COUNT"


const val DIALOG_INPUT = "DIALOG_INPUT"


//SP
const val SP_NAME = "SP"
const val KEY_LOAD_PAGE_TIME = "LOAD_PAGE_TIME"
const val KEY_LOAD_CIRCLE_LAST_INDEX = "KEY_LOAD_CIRCLE_LAST_INDEX"


// WebActivity handler消息key
const val MSG_PAGE_NEXT_EXCEPTION_OR_NOT_FOUNT = 0
const val MSG_TARGET_JUMP_SUC = 1
const val MSG_WEB_VIEW_RECEIVE_ERROR = 2
const val MSG_WEB_VIEW_SET_INDEX = 3
const val MSG_CHECKING_WEB_UPDATE = 5
const val MSG_PAGE_INDEX = 6
const val MSG_PAGE_PIN_INDEX = 7
const val MSG_LOOK_PAGE_DIVS_SMALL = 8
const val CUR_KEYWORD_FINISH = 9
const val NOT_FOUND_NEXT_NODE = 10
const val NOT_FOUND_RESULT_DIV = 11
const val REQUEST_NEXT_TIME_OUT = 12
const val NOT_FOUND_CONTENT = 13

const val KEY_CACHE_LIST = "KEY_CACHE_LIST"


const val HOST_API = "http://47.111.177.58:9110/WebServices/AppUpdateWebService.svc/"
const val HOST_DOWN = "http://47.111.177.58:9110"

//sp
const val LOOP_COUNT = "LOOP_COUNT"
const val SINGLE_LOOP_PAGE_MAX = "SINGLE_LOOP_PAGE_MAX"
const val SWITCH_IP_PAGE_NUM = "SWITCH_IP_PAGE_NUM"
const val PAGE_LOOP_TIME = "PAGE_LOOP_TIME"

const val SINGLE_LOOP_PAGE_MAX_DEFAULT = 5

