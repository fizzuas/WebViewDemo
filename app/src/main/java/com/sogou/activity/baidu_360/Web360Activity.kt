package com.sogou.activity.baidu_360
 ...
 *@description
 */
class Web360Activity : AppCompatActivity() {
    ...
    val baiduIndexUrl = "https://m.so.com/"
    ...
    class MyHandler(activity: Web360Activity) : Handler() {
        private val mActivity: WeakReference<Web360Activity> = WeakReference(activity)
    ...
    private fun setPageIndex(pageIndex: Int) {
        ...
        if (switchIPPages > 0 && (pageIndex > mPinIPPage) && (switchIPPages > 0) && (pageIndex % switchIPPages == 0) && (pageIndex != pageMax)) {
            ...
            GlobalScope.launch(Dispatchers.IO) {
                ...
                for (i in 1..60) {
                    if (NetState.hasNetWorkConnection(this@Web360Activity) && isOnline()) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                if (mLoadingCheckNetDialog == null) {
                    mLoadingCheckNetDialog =
                        JAlertDialog.Builder(this@Web360Activity)
                ...
                if (mKeyWordIndex < mKeyWords.lastIndex) {
                    ...
                    if (mCircleCount == 0) {
                        ...
                        if (mCircleIndex == mCircleCount) {
                            //第mCircleIndex次循环结束
                            ToastUtil.show(this@Web360Activity, "循环结束")
                            finish()
                        } else {
                            //开启下一次循环
                            ToastUtil.show(this@Web360Activity,
