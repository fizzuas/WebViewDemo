package com.sogou.activity.baidu_toutiao
...
import com.sogou.activity.baidu_simplify.TAG_CHECK
import com.sogou.activity.baidu_sougou.UC_CHECK_TIME_INTERVAL
import com.sogou.activity.bean.TUrl
import com.sogou.activity.dialog.JAlertDialog
import com.sogou.activity.util.*
import com.sogou.activity.util.shellutil.CMD
import com.sogou.activity.util.shellutil.ShellUtils
 ...
 *@description
 */
class WebToutTiaoActivity : AppCompatActivity() {
    ...
    val baiduIndexUrl = "http://so.toutiao.com/"
//    val baiduIndexUrl = "http://m.so.com/"
    ...
    class MyHandler(activity: WebToutTiaoActivity) : Handler() {
        private val mActivity: WeakReference<WebToutTiaoActivity> = WeakReference(activity)
    ...
    private fun setPageIndex(pageIndex: Int) {
        Log.e(com.sogou.activity.baidu_simplify.TAG, "page=" + pageIndex)
        ...
        Log.e(com.sogou.activity.baidu_simplify.TAG,
        ...
        if (switchIPPages > 0 && (pageIndex > mPinIPPage) && (switchIPPages > 0) && (pageIndex % switchIPPages == 0) && (pageIndex != pageMax)) {
            ...
            GlobalScope.launch(Dispatchers.IO) {
                ...
                for (i in 1..60) {
                    if (NetState.hasNetWorkConnection(this@WebToutTiaoActivity) && isOnline()) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                if (mLoadingCheckNetDialog == null) {
                    mLoadingCheckNetDialog =
                        JAlertDialog.Builder(this@WebToutTiaoActivity)
                ...
                if (mKeyWordIndex < mKeyWords.lastIndex) {
                    ...
                    if (mCircleCount == 0) {
                        ...
                        if (mCircleIndex == mCircleCount) {
                            //第mCircleIndex次循环结束
                            ToastUtil.show(this@WebToutTiaoActivity, "循环结束")
                            finish()
                        } else {
                            //开启下一次循环
                            ToastUtil.show(this@WebToutTiaoActivity,
        ...
        webview.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url == baiduIndexUrl) {
                    Log.e(MyTag, "头条首页=" + url)
                    //首页，提交表单
                    val jsForm =
                        application.assets.open("toutiao/js_index.js").bufferedReader().use {
                            it.readText()
                        }
...
private class InJavaScriptLocalObj(val context: Context) {
    ...
    fun requestFinished() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            // 页面没有"下一页"，页面异常
            (context as WebToutTiaoActivity).handler.sendEmptyMessage(
                MSG_PAGE_NEXT_EXCEPTION_OR_NOT_FOUNT)
        }
    }
    ...
    fun finish() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            // 目标网页跳转成功
            (context as WebToutTiaoActivity).handler.sendEmptyMessage(MSG_TARGET_JUMP_SUC)
        }
    }
    ...
    fun setItemStartIndex(itemStartIndex: Int) {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            (context as WebToutTiaoActivity).handler.sendMessage(msg)
        }
    }
